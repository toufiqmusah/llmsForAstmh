...
from sentence_transformers import SentenceTransformer  # For 2025, use pre-built vectors
...
class ASTMHClassifier(LightningModule):
    ...
    def __init__(self,
        ...
        self.loss = loss
        self.green_cat_loss = GreenCatNLLLoss()
        self.act = act
        self.lr = lr
        self.t = temperature
        ...
        usePreBuiltEmbeddings = True  # To bypass sentences-transformer
        
        if usePreBuiltEmbeddings:
            input_dim = 768
        else:
            llm = SentenceTransformer(f'sentence-transformers/{LLM_name}')
            input_dim = llm.get_sentence_embedding_dimension()
    ...
    def training_step(self, batch, batch_idx):
        ...
        total_loss, loss, acc, _, _, _ = self._shared_eval_step(batch, batch_idx)

        # Report the loss as train_loss metric
        metrics = {"train_nnlloss": loss,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        Returns:
            loss: float - Loss value computed using the model nn.Module Loss.
        """
        total_loss, loss, acc, gh, ic, m = self._shared_eval_step(batch, batch_idx)
    ...
    def _shared_eval_step(self, batch, batch_idx):
        ...
        return  total_loss, loss, acc, globalhealth_FNplusFP, integratedcontrol_FNplusFP, malaria_FNplusFP
    ...
    def logsoftmaxtemp(self, x):
        ...
        inv_summ = 1 / np.sum(ex.detach().numpy())  # 1/torch.sum(ex, axis=-1)
        # print('\n ex.shape = ' + str(ex.shape)) 
